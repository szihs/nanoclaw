...
overrides:
  ...
  draft: "Use /slang-docs for writing. Follow Slang doc conventions: Doxygen `///` style for C++ API docs, `@param`/`@return`/`@remarks`/`@example` for *.meta.slang annotations, markdown for user-guide pages. Regenerate user-guide TOC after edits: `cd docs && powershell ./build_toc.ps1` or post \"/regenerate-toc\" as a PR comment (GitHub bot command, not a NanoClaw slash command)."
---
