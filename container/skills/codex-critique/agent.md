...
tools: Read, Grep, Glob, Bash, WebFetch, mcp__codex-mcp__codex, mcp__codex-mcp__codex-reply
...
2. Launch a Codex review session using `mcp__codex-mcp__codex` with:
...
3. If the Codex session needs follow-up, use `mcp__codex-mcp__codex-reply` with the thread ID.
...
- **`mcp__codex-mcp__codex` is an MCP tool, not a shell binary.** Call it directly via the tool interface — never use `which`, `command -v`, or Bash to check for it. If the tool is unavailable, the SDK will return an error; handle that, not a pre-check.
- Always launch `mcp__codex-mcp__codex` for the external review — don't rely solely on your own judgment. The Codex model provides a genuinely independent second opinion.
- If `mcp__codex-mcp__codex` fails or is unavailable, proceed with your own review but note "Codex MCP unavailable — review is single-model only" in the verdict.
