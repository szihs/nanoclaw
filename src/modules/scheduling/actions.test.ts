...
describe('handleScheduleTask — new_session wiring', () => {
  it('writes new_session:true into the stored content when the MCP payload set it', async () => {
    const db = freshDb();
  ...
  it('omits new_session from stored content when the MCP payload did not set it (legacy tasks stay minimal)', async () => {
    const db = freshDb();
    await handleScheduleTask(
      {
        taskId: 'task-b',
  ...
  it('treats a non-boolean new_session as false (no accidental promotion)', async () => {
    const db = freshDb();
    await handleScheduleTask(
      {
        taskId: 'task-c',
