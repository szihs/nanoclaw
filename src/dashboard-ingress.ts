...
export function startDashboardIngress(options: DashboardIngressOptions = {}): DashboardIngressHandle {
  ...
  const server = createServer(async (req, res) => {
    ...
    // thread_id: optional. Absent/null/empty-after-trim → root session.
    // Must be a string if present; non-string rejects. Cap at 200 chars so
    // clients can't stuff unbounded blobs into sessions.thread_id.
