...
describe('v2 content guardrails', () => {
  ...
  it('base-nanoclaw skill documents coworker orchestration primitives', () => {
    const source = fs.readFileSync(
      path.join(process.cwd(), 'container', 'skills', 'base-nanoclaw', 'SKILL.md'),
      'utf-8',
    );
    expect(source).toContain('send_message');
    expect(source).toContain('schedule_task');
