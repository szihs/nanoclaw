...
export function getUniqueTrigger(
  ...
  const existing = db
    .prepare('SELECT mga.agent_group_id, mga.trigger_rules FROM messaging_group_agents mga')
    .all() as { agent_group_id: string; trigger_rules: string | null }[];

  const usedPatterns = new Set<string>();
  for (const row of existing) {
    if (excludeAgentGroupId && row.agent_group_id === excludeAgentGroupId) continue;
    if (!row.trigger_rules) continue;
    try {
      const rules = JSON.parse(row.trigger_rules);
      if (rules.pattern) usedPatterns.add(rules.pattern);
    } catch { /* ignore */ }
  }
...
export function ensureDashboardChatWiring(
  ...
  if (!mg) {
    ...
    wdb
      .prepare(
        "INSERT INTO messaging_groups (id, channel_type, platform_id, name, is_group, admin_user_id, created_at) VALUES (?, 'dashboard', ?, ?, 0, NULL, ?)",
      )
  ...
  if (!existingMga) {
    wdb
      .prepare(
        "INSERT INTO messaging_group_agents (id, messaging_group_id, agent_group_id, trigger_rules, response_scope, session_mode, priority, created_at) VALUES (?, ?, ?, ?, 'all', 'shared', 0, ?)",
      )
      .run(
        `mga-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
        mg.id,
        group.id,
        JSON.stringify({ pattern: triggerPattern, requiresTrigger: false }),
        now,
      );
  }
...
function getCoworkerTypes(): Record<string, any> {
  try {
    ...
    const data = JSON.parse(readFileSync(getCoworkerTypesPath(), 'utf-8'));
      ...
      for (const g of registeredGroups) {
        try {
          const mga = db.prepare(
            `SELECT mga.trigger_rules, mg.platform_id
          ...
          if (mga) {
            const rules = mga.trigger_rules ? JSON.parse(mga.trigger_rules) : null;
            g.trigger_pattern = rules?.pattern || null;
...
async function packageV1Archive(
  ...
  const manifest: Record<string, unknown> = {
    ...
    scheduledTasks: v1Tasks.filter(t => t.status === 'active').map(t => ({
      origId: t.id,
      recurrence: t.schedule_type === 'cron' ? t.schedule_value : null,
      processAfter: t.next_run || null,
...
export async function handleRequest(
  ...
  if (req.method === 'GET' && url.pathname.startsWith('/api/memory/')) {
    ...
    try {
      ...
      let content: string;
      if (!raw) {
        // Default: live composed preview — compute on the fly
        try {
          const { composeClaudeMd } = await import('../src/claude-composer.js');
          const rdb = db;
          let coworkerType: string | null = null;
          if (rdb) {
          ...
          content = composeClaudeMd({ manifestName: 'coworker', coworkerType, extraInstructions });
        } catch {
    ...
    if (db) {
      try {
        ...
        for (const g of groups) {
          ...
          // Read composed CLAUDE.md for preview
          try {
            const { composeClaudeMd } = await import('../src/claude-composer.js');
            let coworkerType: string | null = null;
            try {
            ...
            g.memory = composeClaudeMd({ manifestName: 'coworker', coworkerType, extraInstructions });
          } catch {
  ...
  if (req.method === 'POST' && url.pathname === '/api/coworkers') {
    ...
    try {
      ...
      wdb
        .prepare(
          'INSERT INTO agent_groups (id, name, folder, is_admin, agent_provider, container_config, coworker_type, allowed_mcp_tools, created_at) VALUES (?, ?, ?, 0, ?, NULL, ?, ?, ?)',
        )
        .run(agId, name, folder, agentProvider || null, coworkerType, resolvedMcpTools, now);
      ensureDashboardChatWiring(wdb, { id: agId, folder, name }, triggerPattern, now);
  ...
  if (req.method === 'PUT' && /^\/api\/coworkers\/[^/]+$/.test(url.pathname)) {
    ...
    try {
      ...
      // v2: trigger_pattern is in messaging_group_agents.trigger_rules, not agent_groups
  ...
  if (req.method === 'GET' && /^\/api\/coworkers\/[^/]+\/export$/.test(url.pathname)) {
    ...
    try {
      const mgaRow = rdb.prepare(
        "SELECT mga.trigger_rules FROM messaging_group_agents mga JOIN messaging_groups mg ON mg.id = mga.messaging_group_id WHERE mga.agent_group_id = ? LIMIT 1"
      ).get(group.id) as any;
      if (mgaRow?.trigger_rules) {
        const rules = JSON.parse(mgaRow.trigger_rules);
        if (rules.pattern) trigger = rules.pattern;
      }
    } catch { /* use default */ }
    ...
    if (group.coworker_type) {
      requires.coworkerTypes = group.coworker_type.split('+').map((t: string) => t.trim());
      // Walk extends chain to find all required types
      try {
        const typesData = JSON.parse(readFileSync(getCoworkerTypesPath(), 'utf-8'));
        const allRequired = new Set<string>();
        for (const role of (requires.coworkerTypes as string[])) {
          let cur: string | undefined = role;
          while (cur && !allRequired.has(cur)) {
            allRequired.add(cur);
            cur = typesData[cur]?.extends;
          }
        }
        requires.coworkerTypes = [...allRequired];
      } catch { /* use flat list */ }
    }
    // Only record overlays if this agent actually uses composition (has coworkerType)
    if (group.coworker_type) {
      const projectsDir = join(getGroupsDir(), 'templates', 'projects');
      try {
        // Only include overlay dirs that have a coworker-base.md (actually used)
        const overlays = readdirSync(projectsDir).filter(d => {
          try { return existsSync(join(projectsDir, d, 'coworker-base.md')); } catch { return false; }
        });
        if (overlays.length > 0) requires.projectOverlays = overlays;
      } catch { /* none */ }
    }

    // Agent memory files (from container's ~/.claude project memory)
    ...
    if (url.searchParams.get('full') === 'true') {
      try {
        ...
        // Also generate lightweight YAML with archive reference and write to coworkers/
    ...
    // ---- Lightweight YAML export (default) ----
  ...
  if (req.method === 'POST' && url.pathname === '/api/coworkers/import') {
    ...
    if (isBinaryArchive) {
      ...
      try {
        ...
        if (manifest.requires?.coworkerTypes) {
          try {
            const localTypes = JSON.parse(readFileSync(getCoworkerTypesPath(), 'utf-8'));
            for (const t of manifest.requires.coworkerTypes) {
              if (!localTypes[t]) warnings.push(`Missing coworker type: "${t}"`);
            }
          } catch { warnings.push('coworker-types.json not found'); }
        }
        if (manifest.requires?.projectOverlays) {
          const projectsDir = join(getGroupsDir(), 'templates', 'projects');
          for (const overlay of manifest.requires.projectOverlays) {
            if (!existsSync(join(projectsDir, overlay))) warnings.push(`Missing project overlay: "${overlay}"`);
          }
        }
    ...
    try {
      ...
      // Compatibility check: warn if required types/overlays are missing
      const warnings: string[] = [];
      if (isV3 && data.requires) {
        if (data.requires.coworkerTypes) {
          try {
            const localTypes = JSON.parse(readFileSync(getCoworkerTypesPath(), 'utf-8'));
            for (const t of data.requires.coworkerTypes) {
              if (!localTypes[t]) warnings.push(`Missing coworker type: "${t}" — templates may not compose correctly`);
            }
          } catch {
            warnings.push('coworker-types.json not found — typed coworkers may not compose correctly');
          }
        }
        if (data.requires.projectOverlays) {
          const projectsDir = join(getGroupsDir(), 'templates', 'projects');
          for (const overlay of data.requires.projectOverlays) {
            if (!existsSync(join(projectsDir, overlay))) {
              warnings.push(`Missing project overlay: "${overlay}"`);
            }
          }
        }
      }
  ...
  if (req.method === 'POST' && url.pathname === '/api/coworkers/import-v1') {
    ...
    if (body === null) return;
    try {
      const { v1Path, folder } = JSON.parse(body);
      if (!v1Path || !folder) {
        res.writeHead(400, { 'Content-Type': 'application/json' });
          ...
          if (Array.isArray(manifest.scheduledTasks)) {
            ...
            for (const task of manifest.scheduledTasks) {
              inDb.prepare(
                `INSERT INTO messages_in (id, seq, kind, timestamp, status, process_after, recurrence, content)
                 VALUES (?, ?, 'task', ?, 'paused', ?, ?, ?)`
              ).run(task.origId, seq, now, task.processAfter || null, task.recurrence || null,
                typeof task.content === 'string' ? task.content : JSON.stringify(task.content));
              seq += 2;
              tasksImported++;
            }
