    ...
    if (m.cardType === 'ask_question' && m.questionId && m.options && m.options.length > 0) {
      ...
      if (m.isPending) {
        const btns = m.options.map(opt =>
          `<button class="question-btn" data-qid="${esc(m.questionId)}" data-option="${esc(opt)}" style="background:#3B82F6;color:#fff;border:none;border-radius:3px;padding:4px 14px;margin-right:6px;margin-top:4px;cursor:pointer;font-size:10px">${esc(opt)}</button>`
        ).join('');
        return `<div class="cw-msg assistant">
